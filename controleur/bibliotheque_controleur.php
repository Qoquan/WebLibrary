...
if (isset($_POST['enregistrer_note']) && isset($_POST['livre_id'])) {  // ← CORRIGÉ ICI
    ...
    if ($note !== null && ($note < 0 || $note > 5)) {
        ...
        try {
            ...
            if ($result && $stmt->rowCount() > 0) {
                ...
                $messageSucces = "⚠️ Aucune modification effectuée.";
            }
            
        } catch (PDOException $e) {
            $messageSucces = "❌ Erreur lors de l'enregistrement : " . $e->getMessage();
        }
    }
}
...
try {
    ...
    $messageSucces = "❌ Erreur lors de la récupération : " . $e->getMessage();
    $livres = [];
}
?>